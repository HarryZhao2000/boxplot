...
class boxplot:
    def boxplot(ax,
        ...
        for data in Data:
            ...
            if outlier == True:
                for p in outliers:
                    rect = mpathes.Ellipse((center[i], p), 0.04 * center[-1], 0.04 * center[-1], ec=outlier_edgecolor,
            ...
            rect = mpathes.Rectangle((center[i] - 0.2 * center[0], d[0]), 0.4 * center[0], d[-1] - d[0],
                                     ec=box_edgecolor, fc=box_facecolor)
            ax.add_patch(rect)
            i += 1
        plt.show()
    ...
    def hist_boxplot(ax,
        ...
        for data in Data:
            ...
            for c in range(len(Bin)):
                rect = mpathes.Rectangle((center[i], bins[c] + Lower), Bin[c] / 100, w, ec=bin_edgecolor,
                                         fc=bin_facecolor)
                ax.add_patch(rect)
